...
class AddressApp:
    def __init__(self, root):
        ...
        self.ocr_engine = OCREngine(languages=Config.OCR_LANGUAGES, gpu=Config.OCR_GPU)
    ...
    def setup_ui(self):
        ...
        header_frame = tk.Frame(
            self.root, bg=Config.COLORS["bg_secondary"], pady=10, padx=20
        )
    ...
    def check_model_status(self):
        if self.ocr_engine.is_loaded:
            ...
            messagebox.showerror(
                "Ошибка",
                f"Не удалось загрузить OCR модель:\n{self.ocr_engine.load_error}",
            )
        else:
    ...
    def load_image(self):
        ...
        # Process in thread
        threading.Thread(
            target=self.process_image, args=(file_path,), daemon=True
        ).start()
    ...
    def process_image(self, file_path):
        try:
            ...
            self.root.after(
                0, lambda: self.on_process_complete(results, parsed_address)
            )

        except Exception as e:
            error_msg = str(e)
            self.root.after(0, lambda msg=error_msg: self.on_process_error(msg))
    ...
    def update_results(self, data):
        ...
        def update_card(card, text):
            ...
            # So children check
            _ = [c for c in card.winfo_children() if isinstance(c, tk.Label)]
            ...
            content_label.config(
                ...
                font=Config.FONTS["header"] if text else Config.FONTS["normal"],
            )
        ...
        full = []
        if data.get("street_type"):
            full.append(data.get("street_type"))
        if data.get("street_name"):
            full.append(data.get("street_name"))
        if data.get("house_number"):
            full.append(data.get("house_number"))
