...
class AddressParser:
    ...
    def _fix_split_words(texts):
        ...
        while i < len(texts):
            ...
            if i + 2 < len(texts):
                combined = (current + texts[i+1] + texts[i+2]).replace(' ', '').lower()
            ...
            if i + 1 < len(texts):
                ...
                for full_word, variants in split_words.items():
                    for part1, part2 in variants:
                        if (current.lower().strip() == part1 and 
                            texts[i+1].lower().strip() == part2):
    ...
    def parse(self, raw_texts):
        ...
        normalized_texts = [self.normalize_text(t) for t in raw_texts if self.normalize_text(t)]
        ...
        for idx, text in enumerate(normalized_texts):
            ...
            if idx == prefix_index or (found_prefix_normalized and text_lower == found_prefix_normalized):
        ...
        if street_parts:
            ...
            if not result['house_number']:
                ...
                street_name, extracted_house = self._extract_house_from_street(street_name)
                if extracted_house:
                    result['house_number'] = extracted_house
                    print(f"Извлечен номер из названия: {original_name} -> {street_name} + {extracted_house}")
